...
void cPipelineStateObject::Initialize() {
	// RootSignatureの作成
	// Primitive用
	CreatePrimitive3dRootSignature();
	// 3DModel用
	...
	// PSOの作成
	// Primitve用
	CreatePrimitivePipelineStateObject();
	// 3Dmodel用
...
ID3D12PipelineState* cPipelineStateObject::GetPrimitive3dPipelineState(Blendmode blendMode) {
	switch (blendMode) {
		case cPipelineStateObject::kBlendModeNone:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeNone].Get();
			break;
		case cPipelineStateObject::kBlendModeNormal:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeNormal].Get();
			break;
		case cPipelineStateObject::kBlendModeAdd:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeAdd].Get();
			break;
		case cPipelineStateObject::kBlendModeSubtract:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeSubtract].Get();
			break;
		case cPipelineStateObject::kBlendModeMultiply:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeMultiply].Get();
			break;
		case cPipelineStateObject::kBlendModeScreen:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeScreen].Get();
			break;
		default:
			return GetInstance()->primitiveGraphicsPipelineState_[kBlendModeNone].Get();
			break;
...
void cPipelineStateObject::CreatePrimitive3dRootSignature() {
	HRESULT hr = S_FALSE;
	...
	rootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_GEOMETRY;//GeometryShaderで使う
	...
	//バイナリをもとに生成
	primitiveRootSignature_ = nullptr;
	hr = cDirectXCommon::GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
		signatureBlob->GetBufferSize(), IID_PPV_ARGS(&primitiveRootSignature_));
	assert(SUCCEEDED(hr));
...
void cPipelineStateObject::Create3DModelRootSignature() {
	...
	rootParameters[5].Descriptor.ShaderRegister = 3;					//レジスタ番号2とバインド

	rootParameters[6].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;	//CBVを使う
	rootParameters[6].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;	//PixelShaderで使う
	rootParameters[6].Descriptor.ShaderRegister = 4;					//レジスタ番号2とバインド
...
void cPipelineStateObject::CompileShaders() {
	...
	//Shaderをコンパイルする
	// Primitive3d用シェーダー
	primitiveVertexShaderBlob_ = nullptr;
	primitiveVertexShaderBlob_ = CompileShader(L"Engine/Shaders/primitive3d.VS.hlsl",
		L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(primitiveVertexShaderBlob_ != nullptr);

	primitivePixelShaderBlob_ = nullptr;
	primitivePixelShaderBlob_ = CompileShader(L"Engine/Shaders/primitive3d.PS.hlsl",
...
void cPipelineStateObject::CreatePrimitivePipelineStateObject() {
	HRESULT hr;

	assert(primitiveRootSignature_);
	assert(primitiveVertexShaderBlob_);
	assert(primitivePixelShaderBlob_);
	...
	for (uint32_t i = 0; i < kBlendModeNum; i++) {
		...
		primitiveGraphicsPipelineState_[i] = nullptr;
		hr = cDirectXCommon::GetDevice()->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
			IID_PPV_ARGS(&primitiveGraphicsPipelineState_[i]));
		assert(SUCCEEDED(hr));
	}
}
